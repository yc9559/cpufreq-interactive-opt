...
CPU_THREAD = 3
GREEDY_VM = 30             # 100vm about 4.3 min
GREEDY_VM_MULTIPLIER = 60   # to reduce process create overhead
WORKLOAD_LENGTH = 5000
TARGET_IDEAL_RUNTIME = 115
TARGET_PER_SCORE = 67
...
POWER_TABLE_DICT = {
    'phone':    400,
    'tablet':   950,
    's820':     [1.05,130,260,450,640,850,980,1430,1890,2430,3050],      # S820 kryo HP single x 1.3
    's821':     [1.05,180,220,310,480,750,1040,1370,1900,2380,2980],     # S821 kryo HP single x 2, big is the same as LITTLE credit by 叶落情殇 in coolapk
    ...
    '950a72':   [1.1,150,260,380,500,650,850,1120,1300,1650,2100],     # Kirin 950 A72 X2 https://www.anandtech.com/show/9878/the-huawei-mate-8-review/3
    ...
    's801':     [0.8,240,350,520,700,960,1180,1450,1800,2150,2550],     # S800 krait 400 pvs4 single x 2.5
    ...
    's835a53':  [0.8,160,200,310,410,500,570,650,770,870,1000]          # S835 A53 x 3 MI6 credit by 叶落情殇 in coolapk
}
AP = 's835a53'
...
def benchmark(workload, ideal_powersum, above_hispeed_delay, boostpulse_duration, 
    ...
    fps15           = 0.1 * math.sqrt(miss[1] / 300)
    fps30           = 0.05 * math.sqrt(miss[0] / 600)
...
# def exam_exp(result):
#     return -((result[4]-75)**2 + (result[2]-55)**2)
...
def finding_greedy(workload, ideal_powersum):
    ...
    boostpulse_duration_list = range(1,8+1)
    go_hispeed_list = range(39, 99+1, 2)
    hispeed_freq_list = range(300, 900+1, 100)
    min_sampling_time_list = range(1, 4+1)
    target_loads_value_list = range(98, 4, -2)
    ...
    hispeed_freq = 600
    min_sampling_time = 2
    target_loads = [80,80,80,80,80,80,80,80,80,98]
...
if __name__ == "__main__":
    ...
    #             callback = log_result)
    processpool.apply_async(benchmark_thread,
                args = (workload,ideal_powersum,[1, 1, 1, 1, 1, 1, 16, 16, 16, 8],[8],[65],400,[1],[[80, 50, 34, 70, 46, 80, 94, 82, 88, 98]],),
                callback = log_result)
    ...
    csvfile_name = time.strftime("%Y-%m-%d %H%M", time.localtime()) + '-' + AP + '-runtime-' + str(TARGET_IDEAL_RUNTIME) + '-score-' + str(TARGET_PER_SCORE) +'.csv'
    csv_output(csvfile_name,result_buffer)
    # ap_list = ['s821', '7420a57', '7420a53', 's810a57', 's801', 's835a73', 's835a53', '950a72']
    # # ap_list = ['s821']
