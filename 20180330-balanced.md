...
5. 如果使用的是 EX kernel manager，更改调速器设置后开机应用会**自动关闭**
6. 不再强制使用 EX kernel manager，但需要注意 Kernel Auditor 有时候会**CPU占用率100%**
7. MIUI9默认会阻止App开机时自启动，在安全中心**允许自启**
8. 没有提到的参数，或者没有找到的参数，忽略即可
9. 公共部分的HMP参数为**可选**，建议Shell脚本玩家使用
10. 如果有Perfd，请关闭，注意关闭后最低频率自动被设置为最高频率
11. 输入升频：
- 输入间隔设定为50ms
